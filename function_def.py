...
def showHistograms(data, histogram_attributes, cols_number, height, width):
 ...
 # cols and rows start in 1, not in 0, thus we add one.
    col_number = i % cols_number + 1
    row_number = math.ceil((i+1) / cols_number) 
...
def build_encoder_layer(input_shape, encoder_reshape_shape,
                        win_size=10, latent_dim=20):
    # win_size    : Bidirectional LSTM 유닛 수 (원래 전역 win_size)
    # latent_dim  : 잠재공간 차원 (원래 하드코딩 20, = critic_z_input_shape[0])
    x = Input(shape=input_shape)
    model = tf.keras.models.Sequential([
    Bidirectional(LSTM(units=win_size, return_sequences=True)),
    Flatten(),
    Dense(latent_dim), # 잠재공간 차원 = critic_z_input_shape[0]
    Reshape(target_shape=encoder_reshape_shape)]) # (latent_dim, 1)
...
def build_generator_layer(input_shape, generator_reshape_shape,
    ...
    # input_shape = (latent_dim, 1) / generator_reshape_shape = (win_size, 1)
    # win_size     : Dense 유닛 수 (원래 전역 win_size)
    # features_dim : 출력 피처 차원 (원래 전역 features_dim, PCA 축소 차원)
    # lstm_units   : 양방향 LSTM 유닛 수 (원래 하드코딩 64)
    # dropout_rate : 드롭아웃 비율 (원래 하드코딩 0.2)
    x = Input(shape=input_shape)
    model = tf.keras.models.Sequential([
        Flatten(),
        Dense(win_size),
        Reshape(target_shape=generator_reshape_shape),
        Bidirectional(LSTM(units=lstm_units, return_sequences=True), merge_mode='concat'),
...
def build_critic_x_layer(input_shape, k_size=5, filters=64, dropout_rate=0.25):
 # k_size       : Conv1D 커널 크기 (원래 전역 k_size)
 # filters      : Conv1D 필터 수 (원래 하드코딩 64)
 # dropout_rate : 드롭아웃 비율 (원래 하드코딩 0.25)
 x = Input(shape=input_shape)
 model = tf.keras.models.Sequential([
 Conv1D(filters=filters, kernel_size=k_size),
 LeakyReLU(alpha=0.2),
 Dropout(rate=dropout_rate),
 Conv1D(filters=filters, kernel_size=k_size),
 LeakyReLU(alpha=0.2),
...
def critic_x_train_on_batch(x, z, valid, fake, delta,
 ...
 # critic_x_model : 합성 학습 모델 / critic_x : critic_x 서브모델 / optimizer : Adam 등
...
def critic_z_train_on_batch(x, z, valid, fake, delta,
 ...
 # critic_z_model : 합성 학습 모델 / critic_z : critic_z 서브모델 / optimizer : Adam 등
 with tf.GradientTape() as tape:
...
def enc_gen_train_on_batch(x, z, valid,
 ...
 # encoder_generator_model : 인코더-제너레이터 합성 모델 / optimizer : Adam 등
 with tf.GradientTape() as tape:
...
def predict(X, encoder, generator, critic_x, shape, feat_dim):
 """Predict values using the trained models.
 Args:
 X (ndarray): N-dimensional array containing the input sequences for the model.
 encoder, generator, critic_x : 학습된 모델 객체 (원래 전역이던 것)
 shape (tuple): 윈도우 shape, reshape에 shape[0] 사용 (원래 전역 shape)
 feat_dim (int): 피처 차원 (원래 전역 feat_dim)
 Returns:
 ndarray: 각 입력 시퀀스에 대한 재구성값 (y_hat).
 ndarray: 각 입력 시퀀스에 대한 critic 점수.
 """
